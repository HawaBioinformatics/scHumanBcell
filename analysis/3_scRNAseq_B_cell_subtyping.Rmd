...
```{r}
pbmc.44k <- FindVariableFeatures(pbmc.44k, selection.method = "vst", nfeatures = 2000)
```

Identify the 10 most highly variable genes in Bcell subtypes:

```{r}
top10 <- head(VariableFeatures(pbmc.44k), 10)
top10
```
...
table(pbmc.44k@meta.data$CellType)
```
Notice that only B cell subtypes are present

```{r}
# 10k cells
length(colnames(pbmc.44k))
```

```{r}
p1<- DimPlot(pbmc.44k, group.by = "CellType", label = TRUE, repel = TRUE) + NoLegend() + ggtitle("44k pbmc")
```
...
```{r}
transfer.anchors <- FindTransferAnchors(reference = pbmc.44k, query = pbmc, features = VariableFeatures(object = pbmc.44k), 
...
```{r}
celltype.predictions <- TransferData(anchorset = transfer.anchors, refdata = pbmc.44k$CellType, dims = 1:10)
```
```{r}
