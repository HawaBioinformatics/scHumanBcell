...
```{r}
pbmc.26k <- readRDS("/Users/hawacoulibaly/Downloads/SEURAT_OBJECTS/HumanTonsil_BCells_scRNA_SeuratObject.rds")
pbmc.26k <- UpdateSeuratObject(pbmc.26k)
pbmc.26k
```
...
```{r}
pbmc.26k@meta.data %>% head()
```
```{r}
## how many cells for each cell type? 12 B  cell sub-types
table(pbmc.26k@meta.data$CellType)
```
```{r}
# 10k cells
length(colnames(pbmc.26k))
```

```{r}
p1<- DimPlot(pbmc.26k, group.by = "CellType", label = TRUE, repel = TRUE) + NoLegend() + ggtitle("26k pbmc")
```
...
p2<- DimPlot(pbmc, group.by = "seurat_clusters", label = TRUE, repel = TRUE) + NoLegend() + ggtitle("3k pbmc")
...
```{r}
transfer.anchors <- FindTransferAnchors(reference = pbmc.26k, query = pbmc, features = VariableFeatures(object = pbmc.26k), 
...
Let's now identify the subsets using markers B cells subtypes
...
p2<- DimPlot(pbmc, group.by = "seurat_clusters", label = TRUE, repel = TRUE) + NoLegend() + ggtitle("2k pbmc")
...
```{r}
DimPlot(pbmc, group.by = "predicted.id", label = TRUE, repel = TRUE) + NoLegend() + ggtitle("2k pbmc")
```
